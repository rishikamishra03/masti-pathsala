...
export default function MastiPathshalaApp() {
  ...
  const games = [
    { id: 'num', title: "Number Game", icon: "1️⃣", img: numImg, color: "bg-orange-400" },
    { id: 'alpha', title: "Alphabet Game", icon: "🔤", img: alphaImg, color: "bg-cyan-400" },
    { id: 'space', title: "Space Game", icon: "🚀", img: spaceImg, color: "bg-indigo-500" },
    { id: 'word', title: "Words Game", icon: "📝", img: wordImg, color: "bg-pink-400" },
    { id: 'sound', title: "Sound", icon: "🎵", img: soundImg, color: "bg-yellow-400" },
    { id: 'color', title: "Colors", icon: "🎨", img: clrsImg, color: "bg-green-400" },
    { id: 'read', title: "Reading Practice", icon: "📚", img: readImg, color: "bg-red-400" },
    { id: 'math', title: "Maths", icon: "➕", img: mathsImg, color: "bg-blue-500" }
  ];
  ...
  const handleAssistantAction = (action) => {
    switch (action) {
      ...
      case 'NAVIGATE_NUM':
        ...
        break;
      case 'NAVIGATE_POEM':
        // Let's assume it sets view to 'poem' if available, or 'classroom'
        setView('poem');
        break;
      default:
        break;
  ...
  if (view === 'classroom') {
    return (
      <div className="min-h-screen w-full bg-white relative overflow-hidden">
        <VoiceAssistant role="student" userId={currentUser?.id} onAction={handleAssistantAction} />
        <motion.button
          whileHover={{ scale: 1.1 }}
          whileTap={{ scale: 0.9 }}
          onClick={() => setView('hub')}
  ...
  if (view === 'path') return (
    <>
      <MyLearningPaths onBack={() => setView('hub')} />
      <VoiceAssistant role="student" userId={currentUser?.id} onAction={handleAssistantAction} />
    </>
  );
  ...
  if (view === "word") {
    return (
      <div className="fixed inset-0 z-[500] bg-black">
        ...
        <WordsGame onGameOver={(s) => handleGameOver('word', s)} />
      </div>
    );
  }
  ...
  if (view === "poem") {
    return (
      <div className="fixed inset-0 z-[500] bg-[#E3F2FD]">
        <VoiceAssistant role="student" userId={currentUser?.id} onAction={handleAssistantAction} />
        <button
          onClick={() => {
            setView("hub");
        ...
        <div className="pt-20 h-full w-full overflow-y-auto">
          <PoemSection onBack={() => {
            setView("hub");
            audioRef.current?.play().catch(() => { });
          }} />
                ...
                <div className="flex gap-6 overflow-x-auto pb-4 no-scrollbar scroll-smooth px-2">
                  {games.map((game) => (
                    ...
                    >
                      ...
                      <div className="text-orange-500 font-black text-center text-sm">
                        Best: ⭐ {gameScores[game.id] !== undefined ? gameScores[game.id] : 0}
                      </div>
                    </motion.div>
                  ))}
                </div>
